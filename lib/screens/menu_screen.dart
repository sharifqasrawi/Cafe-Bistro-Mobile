...
class _MenuScreenState extends State<MenuScreen> {
  ...
  final _images = [
    'assets/images/menu/s1.jpg',
    'assets/images/menu/s2.jpg',
    'assets/images/menu/s3.jpg',
    'assets/images/menu/s4.jpg',
    'assets/images/menu/s5.jpg',
    'assets/images/menu/s6.jpg',
  ];
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<Menus>(context).fetchAndSetCategories().then((_) {
        menus = Provider.of<Menus>(context).categories;
        final List<Section> allSections2 = <Section>[];

        for (var i = 0; i < menus.length; i++) {
          final section = Section(
            title: menus[i].title,
            leftColor: _colors[i],
            rightColor: _colors[i + 1],
            backgroundAsset: _images[i],
          ...
          for (var j = 0; j < menus[i].items.length; j++) {
            var sectionDetail = SectionDetail(
              ...
              title: menus[i].items[j].title,
              subtitle: menus[i].items[j].description,
              imageAsset: '${menus[i].items[j].price.toStringAsFixed(2)}\$',
              // imageAssetPackage: '',
            );
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : Builder(
              ...
              builder: (ctx) => _buildBody(ctx, _isLoading),
            ),
    );
  }
